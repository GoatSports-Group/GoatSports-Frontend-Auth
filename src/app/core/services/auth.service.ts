...
import { ForgotPasswordRequest, LoginRequest, RegisterRequest, User, VerificationRequest } from '../models/user.model';
...
export class AuthService {
  ...
  login(payload: LoginRequest): Observable<BaseResponse<User>> {
    return this.http.post<BaseResponse<User>>(`${this.apiBase}/api/v1/auth/login`, payload, { withCredentials: true }).pipe(
      tap(response => {
  ...
  loginWithKeycloak(payload: { code: string, redirectUri: string }): Observable<BaseResponse<User>> {
    return this.http.post<BaseResponse<User>>(`${this.apiBase}/api/v1/auth/login/keycloak`, payload, {
  ...
  register(payload: RegisterRequest): Observable<BaseResponse<User>> {
    return this.http.post<BaseResponse<User>>(`${this.apiBase}/api/v1/auth/register`, payload);
  }

  verify(payload: VerificationRequest): Observable<BaseResponse<void>> {
    return this.http.post<BaseResponse<void>>(`${this.apiBase}/api/v1/auth/verify`, payload);
  }

  forgotPassword(payload: ForgotPasswordRequest): Observable<any> {
    return this.http.put<any>(`${this.apiBase}/api/v1/auth/forgot-password`, payload);
  }

  resendVerificationCode(email: string): Observable<any> {
    return this.http.post<any>(`${this.apiBase}/api/v1/auth/resend`, null, {
      params: { email }
    });
  }
